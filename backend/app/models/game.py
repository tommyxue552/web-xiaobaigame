...
from sqlalchemy import (
    Column, Integer, String, Text, Date, DateTime, Index, JSON
)
...
class Game(Base):
    ...
    # ==================== 分类与标签 ====================
    category = Column(String(100), default="", comment="游戏分类（动作/冒险/RPG/策略等）")
    ...
    # ==================== 采集追踪（预留） ====================
    crawler_source = Column(String(100), default="", comment="采集来源标识（如 steam/3dm/gamersky）")
    crawler_url = Column(String(500), default="", comment="采集时的源页面 URL")

    # ==================== 资源中转（预留） ====================
    transfer_status = Column(
        String(50), default="pending",
        comment="中转状态：pending/transferring/completed/failed"
    )
    ...
    publish_status = Column(
        ...
        comment="发布状态：draft/published/hidden"
    )
